...
function import_media_activation_check() {
	if ( version_compare( PHP_VERSION, '5.0.0', '<' ) ) {
		...
		wp_die( "Sorry, but you can't run this plugin, it requires PHP 5 or higher.", 'import-media-pages' );
	}
}
...
// Add option page to admin menu
	$pg = add_options_page( __( 'Media Import', 'import-media-pages' ), __( 'Media Import', 'import-media-pages' ), 'manage_options', basename( __FILE__ ), 'import_media_options_page' );
...
// Help screen 
	...
	$text .= '<p>'.__( "You need to look through the tabs and save your settings before you run the importer. The Tab Tools contains links to some tools that are helpful after you've imported.", 'import-media-pages' ).'</p>';
	
	$text .= '<h3>'.__( 'Tips', 'import-media-pages' )."</h3>
    <ol>
		<li>" . __( "If there is already some content in this site, you should back up your database before you import. Just in case...", 'import-media-pages' )."</li>
    </ol>";
